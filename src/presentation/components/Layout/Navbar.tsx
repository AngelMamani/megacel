...
export const Navbar = ({ onMenuClick }: NavbarProps) => {
  ...
  useEffect(() => {
    const unsubscribe = repositories.notification.subscribe((items) => {
  ...
  const getPageTitle = () => {
    ...
    if (menuItem) {
      return path === '/admin' ? 'Panel Principal' : menuItem.label;
    }
  ...
  const handleNotificationClick = async (id: string) => {
    ...
    try {
      await application.notifications.markAsRead.execute({ notificationId: id });
    } catch {
      // El estado local ya refleja la lectura; Firestore se sincronizará en la suscripción
    }
  };
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      await Promise.all(
        unread.map((notif) =>
          application.notifications.markAsRead.execute({ notificationId: notif.id })
        )
      );
    } catch {
      // La suscripción reconciliará el estado si alguna escritura falla
    }
  };
          ...
          <div className="navbar-user-wrapper" ref={userMenuRef}>
            <div className="navbar-user" onClick={() => setUserMenuOpen(!userMenuOpen)}>
              ...
              <div className="navbar-user-info">
                <span className="navbar-user-name">{user?.name || 'Administrador'}</span>
